...
<!-- Added by: erikpronk, at: Wed May 11 09:52:35 PDT 2022 -->
...
RPK<sub>S,y2</sub> = RPK<sub>S,y1</sub> \* (1 - PctRPKreduction<sub>S,y1</sub>) \* (1 + CAGR<sub>S,y1</sub>)
...
Stock<sub>T,CY,DY</sub>= Stock<sub>T,CY−1,DY </sub>× Survival<sub>T</sub>(CY−DY)
...
Stock<sub>T,CY,DY</sub>= ProjectedFleetSize<sub>T,CY,  </sub>- ΣStock<sub>T,CY</sub>
...
MJ/RPK<sub>Y1</sub> = MJ/RPK<sub>Y0</sub>\* TechnicalEfficiency <sub>Y1</sub> \* PayloadEfficiency <sub>Y1</sub> \* TrafficEfficiency <sub>Y1</sub>
...
MJ \* CO<sub>2</sub>/MJ = CO<sub>2</sub>
...
PriceInc = FuelBurn(L) / TotalPax \* SafPriceDiff ($/L) \* PassThrough

PctPriceInc = (PriceInc + AvgFare) / PriceInc

NewPax = TotalPax \* (1 + Elasticity \* PctPriceInc)
...
**NewTotalFTK = OldTotalFTK \* (1** + **FreightCAGR)**

**NewBellyFTK = FTKPaxRTKRatio \* RPK / 10**
...
NewMJperFTK = OriginalMJperFTK \* TechEfficiencyImprov \* PayloadEfficiencyImprov \* EnRouteEfficiencyImprov

Here OringinalMJperFTK refers to the freight energy intensity in the base year. The EfficiencyImprov values are all relative to the base year efficiencies as well. As mentioned earlier, the module classifies freight carriage according to aircraft class and stage length. Each combination of these two variables has its own efficiency value.
...
(BellyFTK \* MJperFTKBelly + DedicatedFTK \* MJperFTKDedicated) \* CO<sub>2</sub>perMJKersosene
